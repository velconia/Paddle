...
function cmake_gen() {
    mkdir -p ${PADDLE_ROOT}/${BUILD_DIR}
    cd ${PADDLE_ROOT}/${BUILD_DIR}
    ...
    cat <<EOF
    ========================================
    Configuring cmake in /paddle/${BUILD_DIR} ...
...
function check_style() {
    ...
    cp go/glide.* ${BUILD_DIR}/go
    cd ${BUILD_DIR}/go; glide install; cd -
...
function assert_api_not_changed() {
    mkdir -p ${PADDLE_ROOT}/${BUILD_DIR}/.check_api_workspace
    cd ${PADDLE_ROOT}/${BUILD_DIR}/.check_api_workspace
    virtualenv .env
    source .env/bin/activate
    pip install ${PADDLE_ROOT}/${BUILD_DIR}/python/dist/*whl
...
function gen_html() {
    ...
    ========================================
EOF
    export WOBOQ_OUT=${PADDLE_ROOT}/${BUILD_DIR}/woboq_out
    mkdir -p $WOBOQ_OUT
    cp -rv /woboq/data $WOBOQ_OUT/../../data
    /woboq/generator/codebrowser_generator \
    	-b ${PADDLE_ROOT}/${BUILD_DIR} \
    	-a \
    	-o $WOBOQ_OUT \
...
function gen_dockerfile() {
    ...
    cat <<EOF
    ========================================
    Generate ${PADDLE_ROOT}/${BUILD_DIR}/Dockerfile ...
    ========================================
EOF

    cat > ${PADDLE_ROOT}/${BUILD_DIR}/Dockerfile <<EOF
    FROM ${BASE_IMAGE}
...
function gen_capi_package() {
    if [[ ${WITH_C_API} == "ON" ]]; then
        install_prefix="${PADDLE_ROOT}/${BUILD_DIR}/capi_output"
        rm -rf $install_prefix
        make DESTDIR="$install_prefix" install
        cd $install_prefix/usr/local
        ls | egrep -v "^Found.*item$" | xargs tar -cf ${PADDLE_ROOT}/${BUILD_DIR}/paddle.tgz
    fi
}
